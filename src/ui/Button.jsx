import { Link, useNavigate } from "react-router-dom";

// eslint-disable-next-line react/prop-types
function Button({ children, disabled, to, type }) {
  const navigate = useNavigate();
  const base = `inline-block text-sm rounded-full bg-yellow-400 
    font-semibold uppercase tracking-wide text-stone-800
    transition-colors duration-300 hover:bg-yellow-300
    focus:outline-none focus:ring focus:ring-yellow-300 focus:ring-offset-2 disabled:cursor-not-allowed`;

  const styles = {
    primary: base + `px-4 py-3  md:px-6 md:py-4`,
    small: base + `px-4 py-2 md:px-5 md-py-2.5 text-xs`,
    secondary: `rounded-full text-sm border-2 border-stone-200 px-4 py-2.5 font-semibold uppercase
          tracking-wide text-stone-400 transition-colors duration-300 hover:bg-stone-300
          hover:text-stone-800 focus:outline-none focus:ring focus:ring-stone-300
          focus:ring-offset-2 disabled:cursor-not-allowed md:px-6 md:py-3.5`,
  };
  if (to)
    return (
      <Link to={to} className={styles[type] || styles.primary}>
        {children}
      </Link>
    );

  return (
    <button
      disabled={disabled}
      onClick={() => navigate(to)}
      className={styles[type] || styles.primary}
    >
      {children}
    </button>
  );
}

export default Button;
